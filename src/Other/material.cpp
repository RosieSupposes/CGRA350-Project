...
material::material(GLuint shader, GLuint texture, GLuint effectTexture, glm::vec3 color, float sketchedC, float metallic, float roughness)
{
    m_shader = shader;
    m_texture = texture;
...
void material::load(const mat4 &model, const mat4 &view, const mat4 &proj)
{
    ...
    if(isDepthShader)
    {
        glUniformMatrix4fv(glGetUniformLocation(m_shader, "uModelMatrix"), 1, false, value_ptr(model*scale(mat4(1), vec3(1.05f))));
        return;
    }
    ...
    if(m_isDepthTextureSet)
    {
        glUniform1i(glGetUniformLocation(m_shader, "uDepthTexture"), 2);
...
void material::update(float metallic, float roughness, float cValue)
{
	m_metallic = metallic;
	m_roughness = roughness;
	m_sketchedC = cValue;
